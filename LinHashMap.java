       ...
       implements Serializable, Cloneable, Map <K, V>
{
    ...
    private void split ()
    {
        ...
        // Increment mod1 to reflect the new size of the hash table
        mod1++; 
        // Redistribute keys according to the high-resolution hash function 'h2'
        int h2BucketIndex;
        Bucket bh = hTable.get(isplit);
        while (bh != null) 
        {
            for (int j = 0; j < bh.nKeys; j++) 
            {
                K key = bh.key[j];
                V value = bh.value[j];
                h2BucketIndex = h2(key);
                hTable.get(h2BucketIndex).add(key, value);
            }
            bh = bh.next;
        }
        isplit++; // Increment 'isplit'
        // If the current split phase is complete, reset 'isplit' to zero
        if (isplit == mod1) 
        {
            isplit = 0;
            mod1 = mod2;
            mod2 *= 2; // Double mod2 for the next split phase
        }
    } // split
